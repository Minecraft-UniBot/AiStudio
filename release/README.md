...
把「后端 + 前端 + 内置资源 + 内置 opencode」打包为**一个自包含可执行文件**，
运行即自动初始化并启动服务器，无需安装 bun / opencode / 前端依赖。

## 构建

```bash
# 要求 Bun >= 1.4.0（--asset 目录嵌入支持）
bun release/build.ts [--outdir release/artifacts] [--name unibot-studio] [--skip-opencode] [--force-web]
```
...
3. 下载内置 opencode 到 `release/vendor/opencode/`（`--skip-opencode` 跳过，产物回退 PATH）
...
带版本标记，升级后自动重新解压）；默认端口 9876 被占用时自动换空闲端口；
...
├── build.ts                  # 一键打包脚本
├── scripts/fetch-opencode.ts # 下载内置 opencode（npm registry / GitHub Releases）
├── vendor/                   # 打包暂存：内置 opencode（gitignored）

...
- **手动触发**：只构建各平台安装包并上传 artifacts
- **推送 tag `desktop-v*`**（如 `desktop-v0.2.0`）：构建并创建 GitHub Release 草稿，
  版本号取自 tag（`desktop-v0.2.0` → `0.2.0`）
- 矩阵：`macos-14`(arm64) / `windows-latest`(x64) / `ubuntu-24.04`(x64)，Linux ARM64 在注释中预留
- 步骤：装 Bun → 装 Hutch（缓存 `~/.hutch`）→ 安装依赖 → 后端测试 → 构建前端 →
  下载 opencode → stage → `hutch electrobun build --env=production` → 上传 artifacts

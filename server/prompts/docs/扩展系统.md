...
> **关于组合**：`template` 与 `resources` 同属无代码类型，可以组合在同一个扩展包中；`renderer` 是代码型能力，要求独立成包。如需同时发布代码、模板与资源，应拆分为多个扩展包，并通过依赖关系显式连接。
...
- 代码型扩展的入口固定为 `__init__.py`；无代码扩展不需要入口文件。
...
- 模板切换即时生效，无需重启。
- 渲染引擎切换需重启生效；配置的引擎不存在或加载失败时，自动回退默认引擎并记录警告。
